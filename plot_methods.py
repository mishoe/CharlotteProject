...
def plot_regress_corr(true_vals, pred_vals,x_range=None,title='Regression Correlation Plot', dot_size=4, samp_size = 20000,cmap ='RdYlBu_r'):
    ...
    import gc
    train_r = np.sqrt(sklearn.metrics.r2_score(true_vals, pred_vals))

    ## generate samples for sensitivity plots( this needs to go before the importance_frame code )
    rand_inds = np.sort(random.sample(range(len(true_vals)), samp_size))
    plot_x = [true_vals[i] for i in rand_inds]
    plot_y = [pred_vals[i] for i in rand_inds]
    ...
    if x_range is None:
        plt_min = 0
        if int(np.percentile(plot_x, 95))>0:
            plt_max = int(np.percentile(plot_x, 95))
        else:
            plt_max = int(np.percentile(plot_x, 5))
    else:
        plt_min = x_range[0]
        plt_max = x_range[1]
    plt.scatter(plot_x, plot_y, c=z, s=dot_size,cmap=cmap)
    plt.xlabel('True Values', fontsize=12, fontweight='bold')
    plt.ylabel('Predicted Values', fontsize=12, fontweight='bold')
    plt.title(title, fontsize=16)
    plt.text(plt_min + (plt_max-plt_min)*.03, plt_max - (plt_max-plt_min)*.08, "Corr Coef={}".format(round(train_r, 3)), fontsize=12, fontweight='bold')
    plt.ylim((plt_min, plt_max))
    plt.xlim((plt_min, plt_max))
    if plt_max > plt_min:
        plt.plot(range(plt_min, plt_max+1, 1), range(plt_min, plt_max+1, 1), 'k')
    else:
        plt.plot(range(plt_max, plt_min+1, 1), range(plt_max, plt_min+1, 1), 'k')
    plt.show()
    gc.collect()
